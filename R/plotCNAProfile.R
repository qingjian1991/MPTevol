...
plotCNAProfile <- function(cnaqc.list, min_length_show = 1e5) {
  ...
  bl_genome +
    geom_segment(data = calls_flat, aes(x = from, xend = to, y = sample_id, yend = sample_id, color = label), size = 5) +
    scale_color_manual(values = KARYO_colors[2:length(KARYO_colors)]) +
