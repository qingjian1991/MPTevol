...
#' @param maf Maf or MafList object generated by \code{\link[MesKit]{readMaf}} function.
...
#' @param ... Other options passed to \code{\link[MesKit]{subMaf}}
#'
#'
#' @references 1. H index. Hu, Z., et al., Quantitative evidence for early metastatic seeding in colorectal cancer. Nat Genet, 2019. 51(7): p. 1113-1122.
...
calRoutines <- function(maf,
  ...
  processPairCCF <- function(patient.id, ccf.list, JSI.list) {
    ...
    for (i in sample.pairs) {
      ...
      stats[[str_c(pairs, collapse = "-")]] <- nums



      p1 <- Met1_C_OL %>%
        ggplot(aes_string(x = pairs[1], y = pairs[2])) +
        theme_classic2() +
        ...
        geom_abline(slope = 1, intercept = 0, linetype = 3, size = 0.2, alpha = 0.5) +
      ...
      if (subtitle == "both") {
        p1 <- p1 +
          labs(subtitle = sprintf("JSI = %.3f, Hindex = %.3f", JSI.value, Hindex))
      } else if (subtitle == "JSI") {
        p1 <- p1 +
          labs(subtitle = sprintf("JSI = %.3f", JSI.value))
      } else if (subtitle == "Hindex") {
        p1 <- p1 +
          labs(subtitle = sprintf("Hindex = %.3f", Hindex))
      }
      ...
      if (!is.null(maf_drivers)) {
        p1 <- p1 +
          ...
          geom_label_repel(aes(label = Hugo_Symbol),
            data = Met1_C_OL %>% filter(is.driver),
            nudge_x = 0.1, nudge_y = 0.1
          )
      }

      plist[[str_c(pairs, collapse = "-")]] <- p1
    }
