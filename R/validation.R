validMaf <- function(maf_data, remove.empty.VAF = TRUE){
   ...
   #***** modify by CQJ ******
   # retaining mutations with VAF==0, thus the pair-wide comparisons of CCF are optimal.
...
validCCF <- function(ccf_data, maf_data, use.indel.ccf){
    ...
    #***** modify by CQJ ******
    #* add the Cluster information in CCF data.

    if("Cluster" %in% colnames(ccf_data)){

      ccf_data$Cluster = as.integer(ccf_data$Cluster)

      if( all(is.na(ccf_data$Cluster))) {
        warning("Please checking the Cluster column, the Cluster should be integer")
      }else{
        ccf_standardcol = c(ccf_standardcol, "Cluster")
      }
    }
    ...
    keep.columns = c("Patient_ID", "Tumor_Sample_Barcode", "Chromosome", "Start_Position", "CCF")

    if("CCF_Std" %in% colnames(ccf_data)){
      keep.columns = c(keep.columns, "CCF_Std")
    }

    if("Cluster" %in% colnames(ccf_data)){
      keep.columns = c(keep.columns, "Cluster")
    }

    ccf_data <- dplyr::select(ccf_data,
                              dplyr::all_of(keep.columns),
                              dplyr::all_of(indel.ccf.col))
