...
#' @param GeneNames The gene symbols to show. Show the selected genes rather than all genes.
#' @param use.tumorSampleLabel Logical (Default: FALSE). Rename the 'Tumor_Sample_Barcode' with 'Tumor_Sample_Label'.
#'
#' @param Type.name The column to plot the CNVs.
#' @param Type.colors Set the colors.
#' @param rect.patients.size Set the line size of rect to indicate the borders of patients
#'
#' @examples
...
plotCNA <- function(seg,
                    ...
                    Type.name = "Type",
                    Type.colors = setNames( c("#6baed6", "#084594", "#f4a582", "#d73027"), nm = c("Loss", "Deletion",  "Gain",  "Amplification")),
  ...
  ## combine data frame
  if(is(seg, "list")){
    seg <- dplyr::bind_rows(seg) %>% data.table::as.data.table()
  }
  ...
  if(Type.name != "Type"){
    CNADat$Type = seg[seg$Type != "Neutral",] %>% pull(all_of(Type.name))
  }
  ...
  type.all.levels <- names(Type.colors)
  type.all.colors <- Type.colors
  ...
  if(showGene){
    ...
    if(is.null(GeneNames)){
      ...
      gene_table <- CNADat %>%
        distinct(gene_id, .keep_all = TRUE) %>%
        dplyr::filter(Hugo_Symbol %in% GeneNames)
    }
  ...
  if("patient" %in% colnames(seg)&
    ...
    p <- p +
      geom_rect(data = patient.rect.table,
                mapping = aes(xmax = max(chrTable$end), xmin = 0,ymin = hmin, ymax = hmax),
                fill =  NA,
                color = "black",
                size = rect.patients.size
      )
    ...
    p <- cowplot::plot_grid(p + theme(legend.position = "none"),
